...
serve(async (req) => {
  ...
  try {
    const { ingredients, allergies, cuisineType, dietaryPreference } = await req.json()
    ...
    const prompt = `Generate ${numberOfRecipes} different detailed recipes based on the following requirements:
